...
export function AiAnalyse({ architect, engineer, onContinue }: Props) {
  ...
  return (
    <>
      <Topbar title={t("AI-analyse", "AI analysis")} back={false} />
      <div className="view">
        {degraded && (
          <div className="bg-amber-50 border border-amber-100 rounded-xl px-4 py-3">
            <p className="text-sm text-amber-800 leading-snug">
              {t(
                ...
                "The AI service didn't fully respond, so the assessment below uses a rule-based fallback.",
              )}
            </p>
          </div>
        )}
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="font-bold text-[15px]">{t("Teknisk analyse", "Technical analysis")}</p>
            <p className="text-xs text-gray-400">{t("Beregninger etter TEK17 / NS-EN", "Calculations per TEK17 / NS-EN")}</p>
          </div>
        </div>
        ...
        <p className="text-xs text-gray-400 text-center">
          {t("Generert av AI — ikke juridisk bindende", "Generated by AI — not legally binding")}
        </p>

        <div className="mt-auto pt-2">
          <Button size="lg" full onClick={onContinue}>
            {t("Kjøp søknadspakke", "Buy application package")}
            <DownloadIcon />
          </Button>
        </div>
      </div>
