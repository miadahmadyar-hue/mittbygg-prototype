...
export function TilbyggWizard({ p }: { p: Address }) {
  ...
  if (phase.kind !== "wizard") return <ResultPhases phase={phase} setPhase={setPhase} p={p} loadingText="Sjekker SAK10, PBL og reguleringsplanâ€¦" />;
      ...
      <div className="view">
          ...
          <>
            ...
            <div className="space-y-2">
              {(["tilbygg_1etasje", "ny_etasje", "innglasset_terrasse"] as TType[]).map((t) => (
                <RadioCard key={t} selected={data.type === t} onClick={() => setData({ ...data, type: t })} title={LABEL[t]} desc={t === "tilbygg_1etasje" ? "Utvide boligen i grunnflaten" : t === "ny_etasje" ? "Ekstra etasje pÃ¥ toppen" : "Innglasset terrasse eller vinterhage"} />
              ))}
            </div>
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-2">Areal tilbygg (mÂ²)</label>
              <NumberField value={data.areal} onChange={(v) => setData({ ...data, areal: v })} unit="mÂ²" />
              <p className="text-xs text-gray-500 mt-1">â‰¤ 15 mÂ² unntatt Â· 15â€“50 mÂ² sÃ¸knad uten ansvarsrett Â· over 50 mÂ² krever ANS-rett</p>
            </div>
            <div>
              ...
              <p className="text-xs text-gray-500 mt-1">Krav: minst 4 m fra nabogrense (PBL Â§ 29-4)</p>
            </div>
        ...
        {step === 1 && (
          <>
            ...
            <Alert>Vi sjekker mot SAK10, PBL Â§Â§ 20-1, 20-3, 20-4 og Â§29-4 naboavstand.</Alert>
            <div className="mt-auto pt-4 flex flex-col gap-2">
